export class Graph {
    ...
    bar(legends, ...values) {
        ...
        yValues.forEach((value, index) => {
            ...
            this.context.fillRect(index * (barWidth + barSpacing), this.h - barHeight - marginH, barWidth, barHeight);
        });
        this.context.font = '15px Verdana';
        this.context.fillText(legends, this.w * 0.2, this.h * 0.95);
    }
    pie(legends, ...values) {
        const drawRotatingArc = (centerX, centerY, phi, radius, theta, color) => {
            this.context.save();
