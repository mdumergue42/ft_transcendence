...
function getImpact(center, speed, st, vx, height, rec) {
...
function focus(speed, y, width, point) {
    const normalMode = (phi, theta) => {
...
export function ia(ball, self, canvas) {
    var impactSpeed, _;
    if (ball.speed.x <= 0) {
        var st = new Vector2(0, 0);
        var p_y;
        [p_y, impactSpeed] = getImpact(ball.center, ball.speed, st, 1, canvas.height, 15);
        var p_center = new Vector2(0, p_y);
        var p_speed = impactSpeed.clone();
        p_speed.x *= -1;
        [self.obj, impactSpeed] = getImpact(p_center, impactSpeed, self.st, 1, canvas.height, 15);
        self.obj = (self.st.y + self.obj) / 2;
        return;
    }
    [self.obj, impactSpeed] = getImpact(ball.center, ball.speed, self.st, 1, canvas.height, 15);
    var points = [canvas.height, 0];
    self.obj -= focus(impactSpeed.clone(), self.obj, canvas.width, points) * RECT_HEIGHT;
}
