export class AppNavbar extends HTMLElement {
	...
	async connectedCallback() {
		...
		<ul class="list-none p-0 m-0 w-full flex flex-col gap-4" style="height:100%; display: grid; grid-template-rows: repeat(4, auto) 1fr ;">
			${this.navBarEle(true, "/", "home", "Home", "🏠")}
			${this.navBarEle(isLoggedIn,"/match", "match", "Play", "🎮")}
			${this.navBarEle(isLoggedIn,"/history", "history", "History", "📊")}
			${this.navBarEle(isLoggedIn,"/settings", "settings", "Settings", "⚙️")}
			<li class="w-full flex justify-center mt-auto relative">
				<button id="login-btn" class="menu w-14 h-14 flex items-center justify-center bg-black/30 border-2 PBoxBorder rounded-xl PText text-2xl cursor-pointer transition-all duration-300 no-underline PBoxHover hover:scale-110 PShadowHover group" title="Login">
					${isLoggedIn ? "❌": "👤"}
				</button>
				<span class="absolute left-20 bg-black/90 PText border PBoxBorder px-3 py-2 rounded-md text-xs whitespace-nowrap opacity-0 pointer-events-none transition-opacity duration-200 z-[51] font-mono group-hover:opacity-100">Log in</span>
			</li>
		</ul>`;
